...
The default `LLM_PROVIDER=mock` keeps extraction and fit evaluation offline. With `LLM_PROVIDER=gemini`, each uploaded PDF is sent directly to Gemini for structured candidate extraction and fit evaluation; set `GEMINI_API_KEY` and `GEMINI_MODEL` in the environment. API keys must never be committed. Uploaded resumes are stored locally and may contain personal information; this demo is not a production privacy system.
