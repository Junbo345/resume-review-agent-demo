...
function SimpleHRApp() {
  ...
  useEffect(() => { setPendingFiles([]); refreshCandidates().catch(() => setMessage("Unable to load candidates.")); }, [jobId]);
  ...
  const addFiles = (files: FileList | null) => { if (!files?.length) return; setPendingFiles(current => [...current, ...Array.from(files).filter(file => !current.some(existing => existing.name === file.name && existing.size === file.size && existing.lastModified === file.lastModified))]); };
  const upload = async () => { if (!pendingFiles.length || !jobId) return; setBusy(true); setMessage(""); const form = new FormData(); form.append("job_id", jobId); pendingFiles.forEach(file => form.append("files", file)); try { const response = await fetch(`${API}/documents/upload`, { method: "POST", body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.error?.message || "Upload failed"); setUploadResults(data.results || []); setPendingFiles([]); await refreshCandidates(); } catch (error) { setMessage((error as Error).message); } finally { setBusy(false); } };
  ...
  return <Container size="xl" py="xl"><Title order={1}>HR Candidate Workspace</Title><Text c="dimmed" mt="xs">Create a JD, prepare a batch of PDFs, then finish upload to process them together.</Text><Card withBorder mt="xl"><Group align="flex-end"><Select label="Job description" placeholder="Select a JD" value={jobId} onChange={value => value && setJobId(value)} data={jobs.map(item => ({ value: item.id, label: item.title }))} style={{ flex: 1 }} /><Button onClick={() => setShowCreate(value => !value)}>Create JD</Button></Group>{showCreate && <Stack mt="md"><TextInput label="Title" value={title} onChange={event => setTitle(event.currentTarget.value)} /><Textarea label="Description" minRows={5} value={description} onChange={event => setDescription(event.currentTarget.value)} /><Button disabled={!title.trim() || description.trim().length < 20} loading={busy} onClick={createJob}>Save JD</Button></Stack>}</Card>{message && <Alert color="red" mt="md">{message}</Alert>}{currentJob && <><Card withBorder mt="md"><Group justify="space-between"><div><Title order={2}>{currentJob.title}</Title><Text c="dimmed" mt="xs">{currentJob.description}</Text></div><Button color="red" variant="light" loading={busy} onClick={removeJob}>Delete JD</Button></Group><Group mt="lg"><Button component="label" loading={busy}>Add PDF files<input hidden type="file" multiple accept="application/pdf,.pdf" onChange={event => { addFiles(event.currentTarget.files); event.currentTarget.value = ""; }} /></Button><Button disabled={!pendingFiles.length} loading={busy} onClick={upload}>Finish and Upload</Button></Group>{pendingFiles.length > 0 && <Stack mt="md">{pendingFiles.map((file, index) => <Group key={`${file.name}-${index}`} justify="space-between"><Text size="sm">{file.name}</Text><Button size="xs" color="red" variant="subtle" onClick={() => setPendingFiles(files => files.filter((_, fileIndex) => fileIndex !== index))}>Remove</Button></Group>)}</Stack>}{uploadResults.length > 0 && <Stack mt="md"><Text fw={600}>Last upload results</Text>{uploadResults.map((result, index) => <Group key={`${result.filename}-${index}`} justify="space-between"><Text size="sm">{result.filename}</Text><Badge color={result.status === "completed" ? "teal" : result.status === "duplicate" ? "yellow" : "red"}>{result.status}{result.error ? ` · ${result.error}` : ""}</Badge></Group>)}</Stack>}</Card><Card withBorder mt="md"><Title order={2}>Candidates</Title><Text size="sm" c="dimmed" mt="xs">Click a candidate to view all extracted data and the original PDF.</Text><Stack mt="md">{candidates.length === 0 ? <Text c="dimmed">No candidates yet.</Text> : candidates.map(candidate => <Card key={candidate.id} withBorder shadow={selectedCandidate?.id === candidate.id ? "sm" : undefined} style={{ cursor: "pointer" }} onClick={() => setSelectedCandidate(candidate)}><Group justify="space-between"><div><Title order={3}>{candidate.candidate_name || "Unnamed candidate"}</Title><Text size="sm" c="dimmed">{candidate.original_filename}</Text></div><Button size="xs" color="red" variant="subtle" onClick={event => { event.stopPropagation(); removeCandidate(candidate); }}>Delete</Button></Group></Card>)}</Stack></Card>{selectedCandidate && <SimpleGrid cols={{ base: 1, md: 2 }} mt="md"><Card withBorder><Group justify="space-between"><div><Title order={2}>{selectedCandidate.candidate_name || "Unnamed candidate"}</Title><Text size="sm" c="dimmed">{selectedCandidate.original_filename}</Text></div><Button variant="subtle" onClick={() => setSelectedCandidate(null)}>Close</Button></Group><Title order={3} mt="lg">Complete model extraction</Title><Code block mt="md" style={{ maxHeight: "720px", overflow: "auto" }}>{JSON.stringify(selectedCandidate.structured_data || {}, null, 2)}</Code></Card><Card withBorder><Title order={2}>Resume PDF</Title><iframe title="Resume PDF" src={`${API}/documents/${selectedCandidate.document_id}/pdf`} style={{ width: "100%", height: "820px", border: 0 }} /></Card></SimpleGrid>}</>}</Container>;
}
...
function UploadBatch({ jobId, onUploaded }: { jobId: string; onUploaded: () => Promise<void> }) {
  const [files, setFiles] = useState<File[]>([]);
  const [results, setResults] = useState<any[]>([]);
  const [busy, setBusy] = useState(false);
  const addFiles = (selected: FileList | null) => { if (!selected) return; setFiles(current => [...current, ...Array.from(selected).filter(file => !current.some(existing => existing.name === file.name && existing.size === file.size))]); };
  const upload = async () => { if (!files.length) return; setBusy(true); const form = new FormData(); form.append("job_id", jobId); files.forEach(file => form.append("files", file)); try { const response = await fetch(`${API}/documents/upload`, { method: "POST", body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.error?.message || "Upload failed"); setResults(data.results || []); setFiles([]); await onUploaded(); } catch (error) { setResults([{ filename: "Batch", status: "failed", error: (error as Error).message }]); } finally { setBusy(false); } };
  return <Stack mt="lg"><Group><Button component="label" loading={busy}>Add PDF files<input hidden type="file" multiple accept="application/pdf,.pdf" onChange={event => { addFiles(event.currentTarget.files); event.currentTarget.value = ""; }} /></Button><Button disabled={!files.length} loading={busy} onClick={upload}>Finish and Upload</Button></Group>{files.length > 0 && <Stack gap="xs">{files.map((file, index) => <Group key={`${file.name}-${index}`} justify="space-between"><Text size="sm">{file.name}</Text><Button size="xs" color="red" variant="subtle" onClick={() => setFiles(current => current.filter((_, fileIndex) => fileIndex !== index))}>Remove</Button></Group>)}</Stack>}{results.length > 0 && <Stack gap="xs"><Text fw={600}>Last upload results</Text>{results.map((result, index) => <Group key={`${result.filename}-${index}`} justify="space-between"><Text size="sm">{result.filename}</Text><Badge color={result.status === "completed" ? "teal" : result.status === "duplicate" ? "yellow" : "red"}>{result.status}{result.error ? ` · ${result.error}` : ""}</Badge></Group>)}</Stack>}</Stack>;
}

function HiringWorkflowApp() {
  const statuses = ["uploaded", "extracted", "ready_for_review", "gemini_reviewed", "phone_screen", "interview", "offer", "hired", "rejected", "withdrawn"];
  const [jobs, setJobs] = useState<any[]>([]);
  const [jobId, setJobId] = useState("");
  const [candidates, setCandidates] = useState<any[]>([]);
  ...
  const refreshJobs = async () => { const data = await get<any[]>("/jobs"); setJobs(data); if (!jobId && data[0]) setJobId(data[0].id); };
  const refreshCandidates = async () => { if (!jobId) return; const data = await get<any[]>(`/candidates?job_id=${encodeURIComponent(jobId)}`); setCandidates(data); setSelectedCandidate((current: any) => data.find(item => item.id === current?.id) || null); };
  useEffect(() => { refreshJobs().catch(error => setMessage(error.message)); }, []);
  useEffect(() => { setSelectedIds([]); setEvaluation(null); refreshCandidates().catch(error => setMessage(error.message)); }, [jobId]);
  useEffect(() => { if (currentJob) setRubric(currentJob.rubric || ""); }, [jobId, currentJob?.rubric]);
  useEffect(() => { if (!selectedCandidate) { setStatusHistory([]); return; } fetch(`${API}/candidates/${selectedCandidate.id}/status-history`).then(response => response.json()).then(setStatusHistory).catch(error => setMessage(error.message)); }, [selectedCandidate?.id]);

  const create = async () => { setBusy(true); setMessage(""); try { const created = await post<any>("/jobs", { title, description, rubric: rubric.trim() || null }); setJobs(items => [created, ...items]); setJobId(created.id); setTitle(""); setDescription(""); setRubric(""); setShowCreate(false); } catch (error) { setMessage((error as Error).message); } finally { setBusy(false); } };
  const saveRubric = async () => { if (!currentJob) return; setBusy(true); try { const response = await fetch(`${API}/jobs/${currentJob.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rubric: rubric.trim() || null }) }); if (!response.ok) throw new Error("Unable to save rubric"); const updated = await response.json(); setJobs(items => items.map(item => item.id === updated.id ? updated : item)); } catch (error) { setMessage((error as Error).message); } finally { setBusy(false); } };
  const evaluate = async () => { if (!jobId || !selectedIds.length) return; setBusy(true); setMessage(""); try { const data = await post<any>("/reviews/evaluate", { job_id: jobId, candidate_ids: selectedIds }); setEvaluation(data); await refreshCandidates(); } catch (error) { setMessage((error as Error).message); } finally { setBusy(false); } };
  const updateStatus = async (candidate: any, status: string) => { try { const response = await fetch(`${API}/candidates/${candidate.id}/status`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) }); if (!response.ok) throw new Error("Unable to update status"); await refreshCandidates(); } catch (error) { setMessage((error as Error).message); } };
  const removeCandidate = async (candidate: any) => { if (!window.confirm(`Delete ${candidate.candidate_name || candidate.original_filename}?`)) return; const response = await fetch(`${API}/documents/${candidate.document_id}`, { method: "DELETE" }); if (!response.ok) setMessage("Unable to delete candidate"); else { if (selectedCandidate?.id === candidate.id) setSelectedCandidate(null); await refreshCandidates(); } };
  const visibleCandidates = statusFilter === "all" ? candidates : candidates.filter(candidate => candidate.current_status === statusFilter);

  return <Container size="xl" py="xl"><Title order={1}>HR Candidate Workflow</Title><Text c="dimmed" mt="xs">Manage candidates by job, optionally add a rubric, and send selected candidates to Gemini for evaluation.</Text><Card withBorder mt="xl"><Group align="flex-end"><Select label="Job" placeholder="Select a job" value={jobId} onChange={value => value && setJobId(value)} data={jobs.map(item => ({ value: item.id, label: item.title }))} style={{ flex: 1 }} /><Button onClick={() => setShowCreate(value => !value)}>Create job</Button></Group>{showCreate && <Stack mt="md"><TextInput label="Job title" value={title} onChange={event => setTitle(event.currentTarget.value)} /><Textarea label="Job description" minRows={5} value={description} onChange={event => setDescription(event.currentTarget.value)} /><Textarea label="Rubric (optional)" description="If provided, Gemini follows these criteria. If empty, Gemini scores general fit from the JD." minRows={4} value={rubric} onChange={event => setRubric(event.currentTarget.value)} /><Button disabled={!title.trim() || description.trim().length < 20} loading={busy} onClick={create}>Save job</Button></Stack>}</Card>{message && <Alert color="red" mt="md">{message}</Alert>}{currentJob && <><Card withBorder mt="md"><Group justify="space-between"><div><Title order={2}>{currentJob.title}</Title><Text c="dimmed" mt="xs">{currentJob.description}</Text></div><Button color="red" variant="light" onClick={async () => { if (!window.confirm(`Delete job "${currentJob.title}" and its candidates?`)) return; await fetch(`${API}/jobs/${currentJob.id}`, { method: "DELETE" }); const remaining = jobs.filter(item => item.id !== currentJob.id); setJobs(remaining); setJobId(remaining[0]?.id || ""); }}>Delete job</Button></Group><Textarea label="Rubric (optional)" description="Sent to Gemini only when non-empty." mt="lg" minRows={4} value={rubric} onChange={event => setRubric(event.currentTarget.value)} /><Button mt="sm" variant="light" loading={busy} onClick={saveRubric}>Save rubric</Button><UploadBatch jobId={jobId} onUploaded={refreshCandidates} /></Card><Card withBorder mt="md"><Group justify="space-between"><div><Title order={2}>Candidates</Title><Text size="sm" c="dimmed">Select candidates for Gemini, or click one to inspect it.</Text></div><Group><Select aria-label="Filter candidates by hiring status" value={statusFilter} onChange={value => setStatusFilter(value || "all")} data={[{ value: "all", label: "All statuses" }, ...statuses.map(status => ({ value: status, label: status }))]} /><Button variant="subtle" onClick={() => setSelectedIds(visibleCandidates.filter(item => item.extraction_status === "completed").map(item => item.id))}>Select all</Button><Button disabled={!selectedIds.length} loading={busy} onClick={evaluate}>Evaluate selected</Button></Group></Group><Stack mt="md">{visibleCandidates.length === 0 ? <Text c="dimmed">No candidates match this filter.</Text> : visibleCandidates.map(candidate => <Card key={candidate.id} withBorder shadow={selectedCandidate?.id === candidate.id ? "sm" : undefined} style={{ cursor: "pointer" }} onClick={() => setSelectedCandidate(candidate)}><Group align="flex-start"><Checkbox checked={selectedIds.includes(candidate.id)} onChange={event => { event.stopPropagation(); setSelectedIds(ids => event.currentTarget.checked ? [...ids, candidate.id] : ids.filter(id => id !== candidate.id)); }} onClick={event => event.stopPropagation()} /><div style={{ flex: 1 }}><Group justify="space-between"><div><Title order={3}>{candidate.candidate_name || "Unnamed candidate"}</Title><Text size="sm" c="dimmed">{candidate.original_filename}</Text></div><Group><Badge>{candidate.current_status}</Badge>{candidate.structured_data.__latest_fit && <Badge color="indigo">{candidate.structured_data.__latest_fit.score}/100</Badge>}<Button size="xs" color="red" variant="subtle" onClick={event => { event.stopPropagation(); removeCandidate(candidate); }}>Delete</Button></Group></Group><Select mt="sm" label="Hiring status" value={candidate.current_status} data={statuses.map(status => ({ value: status, label: status }))} onChange={value => value && updateStatus(candidate, value)} onClick={event => event.stopPropagation()} /></div></Group></Card>)}</Stack></Card>{selectedCandidate && <SimpleGrid cols={{ base: 1, md: 2 }} mt="md"><Card withBorder><Group justify="space-between"><div><Title order={2}>{selectedCandidate.candidate_name || "Unnamed candidate"}</Title><Text size="sm" c="dimmed">{selectedCandidate.original_filename}</Text></div><Badge>{selectedCandidate.current_status}</Badge></Group><Title order={3} mt="lg">Complete extraction</Title><Code block mt="sm" style={{ maxHeight: "500px", overflow: "auto" }}>{JSON.stringify(selectedCandidate.structured_data, null, 2)}</Code>{selectedCandidate.structured_data.__latest_fit && <><Title order={3} mt="lg">Latest Gemini evaluation</Title><Code block mt="sm">{JSON.stringify(selectedCandidate.structured_data.__latest_fit, null, 2)}</Code></>}<Title order={3} mt="lg">Status history</Title>{statusHistory.length ? <Stack gap="xs" mt="sm">{statusHistory.map(item => <Text size="sm" key={item.id}>{item.from_status || "new"} → {item.to_status} · {new Date(item.changed_at).toLocaleString()}</Text>)}</Stack> : <Text size="sm" c="dimmed" mt="sm">No status history.</Text>}</Card><Card withBorder><Title order={2}>Resume PDF</Title><iframe title="Resume PDF" src={`${API}/documents/${selectedCandidate.document_id}/pdf`} style={{ width: "100%", height: "820px", border: 0 }} /></Card></SimpleGrid>}</>}</Container>;
}
