...
function AccordionPreview({ data }: { data: any }) { const [deleting, setDeleting] = useState(false); const documentId = data.__document_id as string | undefined; const visible = { ...data }; delete visible.__document_id; const remove = async () => { if (!documentId || !window.confirm("Delete this saved resume and its extracted data?")) return; setDeleting(true); try { const response = await fetch(`${API}/documents/${documentId}`, { method: "DELETE" }); if (!response.ok) throw new Error("Delete failed"); window.location.reload(); } finally { setDeleting(false); } }; return <><details className="structured-preview"><summary>View structured candidate data</summary><Code block mt="sm">{JSON.stringify(visible, null, 2)}</Code></details>{documentId && <Group justify="flex-end" mt="sm"><Button size="xs" color="red" variant="subtle" loading={deleting} leftSection={<IconTrash size={14} />} onClick={remove}>Delete resume</Button></Group>}</>; }
